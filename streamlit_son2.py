...
tab1, tab2, tab3 = st.tabs(["Title", "Variety", "Unsupervised"])
wine_df=pd.read_csv("/Users/havvaserim/Desktop/mywineproject/Streamlit/preprocessed_wine_df_10_01_23.csv", index_col="Unnamed: 0")
...
def recom_by_title (na, df, number=5):
    ...
    if st.button('Recommend my Wine with Title'):
        ...
        recom_by_title(title_option, df)
...
#Checked
variety_description_2.head()
#Oluşan df sadece 1'den fazla yorumlananları içeriyor.
#Bir kez yorumlananları da eklemek için:
...
#Veri seti, TF- IDF analizi için hazırlanmış oldu.
...
def recommend_by_variety(grape, cosine_sim=cosine_sim):
    ...
    if st.button('Recommend my Wine with Variety'):
        stprint("Recommended with variety")
        recommend_by_variety(variety_option)
...
user_input = st.text_input("Enter key words for wine i.e fresh, red, black, tannin etc.",'')
...
df=pd.read_csv("/Users/havvaserim/Desktop/mywineproject/Streamlit/preprocessed_wine_df_10_01_23.csv", index_col="Unnamed: 0")

desc = df.reset_index(drop=True)

st.write(desc["description"][0].split())

seq = difflib.SequenceMatcher()

st.write(seq.set_seqs(ww,desc["description"][1].split()))
st.write("ratio",seq.ratio()*100)
